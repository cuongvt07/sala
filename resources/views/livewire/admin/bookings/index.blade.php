        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50/50">
                <tr>
                    ...
                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Tiền phòng</th>
                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Cọc</th>
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">
                            <div class="flex flex-col">
                                ...
                                <span class="text-xs text-gray-400">In: {{ $booking->check_in ? $booking->check_in->format('d/m/Y') : '-' }}</span>
                                <span class="text-xs {{ $booking->price_type == 'month' && !$booking->check_out ? 'text-red-500' : 'text-gray-500' }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            <div class="flex flex-col">
                                ...
                                @if($booking->unit_price > 0)
                                    <span class="text-[10px] text-gray-400 uppercase font-semibold">Đơn giá: {{ number_format($booking->unit_price, 0, ',', '.') }}đ</span>
                                @endif
                            </div>
                        </td>
            ...
            <div class="w-full lg:w-52 bg-slate-950 border-r border-slate-900 flex flex-col p-2 gap-1">
                ...
                @foreach($tabs as $tab)
                    <button type="button" 
                        ...
                        class="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold transition-all duration-200 {{ $activeModalTab === $tab['id'] ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:bg-slate-900 hover:text-white' }}">
                ...
                <div class="mt-auto pt-2 border-t border-slate-900">
                    <div class="p-2 bg-slate-900/50 rounded-lg">
                        <p class="text-[9px] text-slate-500 uppercase font-black tracking-widest mb-1.5 px-1">Trạng thái</p>
                        <select wire:model="status" class="w-full bg-slate-900 border-slate-700 text-white text-[11px] font-bold rounded p-1.5 focus:ring-blue-600 focus:border-blue-500 transition-all">
            ...
            <div class="flex-1 flex flex-col bg-white overflow-hidden">
                <form wire:submit="save" class="flex flex-col h-full bg-gray-50/10">
                    <div class="flex-1 overflow-y-auto p-5">
                        ...
                        <div x-show="$wire.activeModalTab === 'overview'" class="space-y-4">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <div class="bg-indigo-600 p-4 rounded-xl text-white shadow-lg overflow-hidden relative">
                                    ...
                                    <p class="text-[9px] font-black uppercase tracking-widest mb-1 opacity-80">Tổng cộng hóa đơn</p>
                                    ...
                                    <div class="mt-2 text-[10px] bg-black/20 p-1.5 rounded-lg flex justify-between items-center">
                                ...
                                <div class="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex flex-col justify-center">
                                    <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-2 px-1">Phân bổ nguồn thu</p>
                            ...
                            <div class="bg-white p-4 rounded-xl border border-gray-100 shadow-sm space-y-3">
                                <h4 class="text-[10px] font-black text-gray-400 uppercase tracking-widest border-b pb-2 flex items-center gap-2">
                                ...
                                <div class="grid grid-cols-2 gap-y-2 text-xs">
                                    ...
                                    <div class="font-bold text-gray-900 text-right">{{ \Carbon\Carbon::parse($check_in)->format('d/m/Y') }} → {{ $check_out ? \Carbon\Carbon::parse($check_out)->format('d/m/Y') : '...' }}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div x-show="$wire.activeModalTab === 'customer'" class="space-y-4">
                            <div class="flex items-center justify-between">
                                <h3 class="text-sm font-black text-gray-900 uppercase tracking-tight">Thông tin Khách</h3>
                                <button type="button" wire:click="$set('activeTab', '{{ $activeTab === 'existing' ? 'new' : 'existing' }}')"
                                    class="text-[10px] font-black py-1.5 px-3 rounded bg-gray-100 text-gray-600 uppercase hover:bg-blue-600 hover:text-white transition-all">
                            ...
                            @if($activeTab === 'existing')
                                 <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm space-y-3">
                                    <select wire:model.live="customer_id" class="block w-full rounded-lg border-gray-200 bg-white p-2.5 text-xs font-bold text-gray-900 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all shadow-sm">
                                    ...
                                    @if($customer_id && ($selectedCustomer = $customers->find($customer_id)))
                                        <div class="pt-3 border-t border-gray-100 grid grid-cols-2 gap-3">
                                            <div class="p-2 bg-gray-50 rounded-lg">
                                                <p class="text-[9px] text-gray-400 font-bold uppercase mb-0.5">SĐT</p>
                                                <p class="text-xs font-bold text-gray-900">{{ $selectedCustomer->phone }}</p>
                                            </div>
                                            <div class="p-2 bg-gray-50 rounded-lg">
                                                <p class="text-[9px] text-gray-400 font-bold uppercase mb-0.5">Identity</p>
                                                <p class="text-xs font-bold text-gray-900">{{ $selectedCustomer->identity ?: 'N/A' }}</p>
                                            </div>
                                        </div>
                                    @endif
                                </div>
                            @else
                                <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm grid grid-cols-2 gap-3">
                                    ...
                                    <div class="col-span-2 space-y-1">
                                         <label class="text-[10px] font-bold text-gray-400 uppercase">Ảnh Visa/Passport</label>
                                         <input type="file" wire:model="new_customer_image" class="block w-full text-[10px] text-gray-500 file:mr-4 file:py-1 file:px-4 file:rounded file:border-0 file:text-[10px] file:font-black file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                                    </div>
                                </div>
                            @endif
                        </div>
                        ...
                        <div x-show="$wire.activeModalTab === 'room'" class="space-y-4">
                            <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm space-y-4">
                                <div class="space-y-1">
                                    <label class="text-[10px] font-bold text-gray-400 uppercase">Chọn phòng</label>
                                    <select wire:model.live="room_id" class="block w-full rounded-lg border-gray-200 bg-white p-2.5 text-sm font-black text-gray-900 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all shadow-sm">
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    <div class="col-span-2 p-3 bg-gray-50 rounded-lg space-y-2">
                                        <label class="text-[10px] font-bold text-gray-400 uppercase">Loại hình thuê</label>
                                        <div class="flex p-0.5 bg-gray-200 rounded-lg">
                                            <button type="button" wire:click="$set('price_type', 'day')" class="flex-1 py-1.5 rounded-md text-[10px] font-black uppercase {{ $price_type === 'day' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500' }}">Ngày</button>
                                            <button type="button" wire:click="$set('price_type', 'hour')" class="flex-1 py-1.5 rounded-md text-[10px] font-black uppercase {{ $price_type === 'hour' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500' }}">Giờ</button>
                                            <button type="button" wire:click="$set('price_type', 'month')" class="flex-1 py-1.5 rounded-md text-[10px] font-black uppercase {{ $price_type === 'month' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500' }}">Tháng</button>
                                        </div>
                                    </div>
                                ...
                                @foreach($all_services as $service)
                                    <div wire:click="toggleService({{ $service->id }})" class="p-2.5 rounded-xl border transition-all cursor-pointer flex items-center justify-between {{ !empty($selected_services[$service->id]['selected']) ? 'border-blue-600 bg-blue-50/50' : 'border-gray-100 hover:border-gray-300' }}">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <div>
                                                <p class="text-[11px] font-bold text-gray-900 leading-tight">{{ $service->name }}</p>
                                                <p class="text-[9px] text-gray-400 font-bold uppercase">{{ number_format($service->unit_price, 0, ',', '.') }}đ</p>
                                            </div>
                                        </div>
                        ...
                        <div x-show="$wire.activeModalTab === 'history'" class="space-y-4">
                            <div class="bg-white p-4 rounded-xl border border-gray-200 space-y-3">
                                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest border-b pb-1.5 flex items-center justify-between">
                                    ...
                                    @if($hasSel)
                                        @foreach($all_services as $service)
                                            @if(!empty($selected_services[$service->id]['selected']) && isset($service_inputs[$service->id]))
                                                <div class="p-2 border rounded-lg hover:border-blue-300 transition-all bg-gray-50/30">
                                                    <div class="grid grid-cols-2 lg:grid-cols-6 gap-2 items-end">
                                                        <div class="lg:col-span-1 pb-1.5"><p class="text-[10px] font-black text-blue-600 uppercase truncate">{{ $service->name }}</p></div>
                                                        @if($service->type === 'meter')
                                                            <div class="space-y-0.5"><label class="text-[8px] text-gray-400 uppercase font-black">Số đầu</label><input type="number" wire:model="service_inputs.{{ $service->id }}.start_index" class="w-full text-[11px] font-bold border-gray-100 bg-gray-50/50 text-gray-400 rounded p-1 h-7 cursor-not-allowed" readonly></div>
                                                            <div class="space-y-0.5"><label class="text-[8px] text-gray-400 uppercase font-black">Số mới</label><input type="number" wire:model="service_inputs.{{ $service->id }}.end_index" class="w-full text-[11px] font-bold border-blue-200 bg-white rounded p-1 h-7 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all"></div>
                                                        @else
                                                            <div class="space-y-0.5 lg:col-span-2"><label class="text-[8px] text-gray-400 uppercase font-black">S.Lượng ({{ $service->unit_name }})</label><input type="number" wire:model="service_inputs.{{ $service->id }}.quantity" class="w-full text-[11px] font-bold border-gray-200 bg-white rounded p-1 h-7 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all"></div>
                                                        @endif
                                                        <div class="space-y-0.5"><label class="text-[8px] text-gray-400 uppercase font-black">Đơn giá</label><input type="text" wire:model.blur="service_inputs.{{ $service->id }}.unit_price" class="w-full text-[11px] font-bold border-gray-200 bg-white rounded p-1 h-7 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all" x-on:input="$el.value = $el.value.replace(/[^0-9]/g, '').replace(/\B(?=(\d{3})+(?!\d))/g, '.')"></div>
                                                        <div class="flex flex-col items-center pb-1"><p class="text-[8px] text-gray-400 uppercase font-black">Tạm tính</p><p class="text-[10px] font-black text-blue-600">@php $inp = $service_inputs[$service->id] ?? []; $up = (float)str_replace(['.',','],'',$inp['unit_price']??'0'); if($service->type==='meter'){ $rt = max(0, ((float)($inp['end_index']??0) - (float)($inp['start_index']??0))) * $up; }else{ $rt = ((float)($inp['quantity'] ?? 1)) * $up; } @endphp {{ number_format($rt, 0, ',', '.') }}đ</p></div>
                                                        <div class="pb-0.5"><button type="button" wire:click="addServiceLog({{ $service->id }})" class="w-full bg-blue-600 text-white rounded text-[9px] font-black uppercase py-1.5 hover:bg-blue-700 shadow-sm active:scale-95 transition-all">Chốt</button></div>
                                                    </div>
                                                </div>
                                            @endif
                                        @endforeach
                                    @else
                                        <p class="text-[10px] text-gray-400 italic text-center py-2">Chọn dịch vụ trước</p>
                                    @endif

                                    <!-- MANUAL SURCHARGE ROW -->
                                    <div class="pt-2 border-t border-dashed border-gray-200 mt-2">
                                        <p class="text-[8px] font-black text-indigo-400 uppercase tracking-widest mb-1.5 px-1 flex items-center gap-1">
                                        ...
                                        <div class="grid grid-cols-12 gap-1.5 bg-indigo-50/50 p-2 rounded-lg border border-indigo-100/50">
                                            <div class="col-span-4 space-y-0.5">
                                                <label class="text-[8px] text-gray-400 uppercase font-black">Số tiền</label>
                                                <input type="text" wire:model.blur="manual_fee_amount" class="w-full text-[11px] font-bold border-indigo-200 bg-white rounded p-1 h-7" placeholder="Số tiền..." x-on:input="$el.value = $el.value.replace(/[^0-9]/g, '').replace(/\B(?=(\d{3})+(?!\d))/g, '.')">
                                            </div>
                                            <div class="col-span-6 space-y-0.5">
                                                <label class="text-[8px] text-gray-400 uppercase font-black">Ghi chú phí</label>
                                                <input type="text" wire:model="manual_fee_notes" class="w-full text-[11px] font-bold border-indigo-100 bg-white rounded p-1 h-7" placeholder="Lý do phụ thu...">
                                            </div>
                                            <div class="col-span-2 flex items-end pb-0.5">
                                                <button type="button" wire:click="addManualSurcharge" class="w-full bg-indigo-600 text-white rounded text-[9px] font-black uppercase py-1.5 hover:bg-indigo-700 shadow-sm active:scale-95 transition-all">Lưu</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <div class="group flex items-center gap-2 bg-white px-2 py-1.5 rounded-lg border border-gray-100 hover:border-indigo-200 shadow-sm transition-all animate-fadeIn">
                                            <div class="flex-1">
                                                <div class="flex justify-between items-center mb-0.5">
                                                    <p class="text-[10px] font-black text-gray-900 uppercase tracking-tight">{{ $log['service_name'] }}</p>
                                                    <p class="text-[10px] font-black text-indigo-600">{{ number_format($log['total_amount'], 0, ',', '.') }}đ</p>
                                                </div>
                                                <div class="flex gap-2 text-[8px] text-gray-400 font-bold items-center">
                                                    ...
                                                    @elseif($log['type'] === 'manual')
                                                        <span class="px-1 bg-indigo-50 text-indigo-500 rounded uppercase text-[7px]">Surcharge</span>
                                                    @else 
                    ...
                    <div class="px-5 py-3.5 bg-white border-t border-gray-100 flex justify-end gap-2 items-center">
                        <button type="button" @click="show = false" class="px-4 py-2 text-xs font-bold text-gray-500 hover:bg-gray-100 rounded-lg transition-all">Huỷ</button>
                        <button type="submit" class="px-8 py-2.5 bg-blue-600 text-white text-xs font-black uppercase tracking-widest rounded-lg shadow-lg shadow-blue-100 hover:bg-blue-700 active:scale-95 transition-all flex items-center gap-2">
