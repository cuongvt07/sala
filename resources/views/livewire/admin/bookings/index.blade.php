            ...
            <div class="w-full lg:w-52 bg-slate-950 border-r border-slate-900 flex flex-col p-2 gap-1">
                ...
                @foreach($tabs as $tab)
                    <button type="button" 
                        ...
                        class="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold transition-all duration-200 {{ $activeModalTab === $tab['id'] ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-900 hover:text-white' }}">
                    ...
                    <div class="flex-1 overflow-y-auto p-5">
                        ...
                        <div x-show="$wire.activeModalTab === 'overview'" class="space-y-4">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <div class="bg-indigo-600 p-4 rounded-xl text-white shadow-lg overflow-hidden relative">
                                    ...
                                    <div class="mt-2 text-[12px] bg-black/20 p-1.5 rounded-lg flex justify-between items-center">
                            ...
                            <div class="bg-white p-4 rounded-xl border border-gray-100 shadow-sm space-y-3">
                                <h4 class="text-[12px] font-black text-gray-900 uppercase tracking-widest border-b pb-2 flex items-center gap-2">
                                    <div class="w-1 h-1 rounded-full bg-blue-600"></div>
                                    Tóm tắt thông tin
                                </h4>
                                <div class="grid grid-cols-2 gap-y-2 text-[12px]">
                                    <div class="text-gray-900 font-medium italic">Phòng:</div>
                                    <div class="font-bold text-gray-900 text-right">{{ $rooms->find($room_id)?->code ?? 'Chưa chọn' }}</div>
                                    <div class="text-gray-900 font-medium italic">Khách hàng:</div>
                                    ...
                                    <div class="text-gray-900 font-medium italic">Thời gian:</div>
                        ...
                        <div x-show="$wire.activeModalTab === 'customer'" class="space-y-4">
                            <div class="flex items-center justify-between">
                                ...
                                <button type="button" wire:click="$set('activeTab', '{{ $activeTab === 'existing' ? 'new' : 'existing' }}')"
                                    class="text-[12px] font-black py-1.5 px-3 rounded bg-gray-100 text-gray-600 uppercase hover:bg-blue-600 hover:text-white transition-all">
                            ...
                            @if($activeTab === 'existing')
                                 <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm space-y-3">
                                    <select wire:model.live="customer_id" class="block w-full rounded-lg border-gray-200 bg-white p-2.5 text-[12px] font-bold text-gray-900 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all shadow-sm">
                                    ...
                                    @if($customer_id && ($selectedCustomer = $customers->find($customer_id)))
                                        <div class="pt-3 border-t border-gray-100 grid grid-cols-2 gap-3">
                                            <div class="p-2 bg-gray-50 rounded-lg">
                                                <p class="text-[11px] text-gray-900 font-bold uppercase mb-0.5">SĐT</p>
                                                <p class="text-[12px] font-bold text-gray-900">{{ $selectedCustomer->phone }}</p>
                                            </div>
                                            <div class="p-2 bg-gray-50 rounded-lg">
                                                <p class="text-[11px] text-gray-900 font-bold uppercase mb-0.5">Identity</p>
                                                <p class="text-[12px] font-bold text-gray-900">{{ $selectedCustomer->identity ?: 'N/A' }}</p>
                                            </div>
                                        </div>
                                    @endif
                                </div>
                            @else
                                <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm grid grid-cols-2 gap-3">
                                    ...
                                    <div class="col-span-2 space-y-1">
                                         <label class="text-[12px] font-bold text-gray-900 uppercase">Ảnh Visa/Passport</label>
                                         <input type="file" wire:model="new_customer_image" class="block w-full text-[12px] text-gray-900 file:mr-4 file:py-1 file:px-4 file:rounded file:border-0 file:text-[11px] file:font-black file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                                    </div>
                                </div>
                            @endif
                        </div>
                                ...
                                @foreach($all_services as $service)
                                    <div wire:click="toggleService({{ $service->id }})" class="p-2.5 rounded-xl border transition-all cursor-pointer flex items-center justify-between {{ !empty($selected_services[$service->id]['selected']) ? 'border-blue-600 bg-blue-50/50' : 'border-gray-100 hover:border-gray-300' }}">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <div>
                                                <p class="text-[12px] font-bold text-gray-900 leading-tight">{{ $service->name }}</p>
                                                <p class="text-[11px] text-gray-900 font-bold uppercase">{{ number_format($service->unit_price, 0, ',', '.') }}đ</p>
                                            </div>
                                        </div>
                        ...
                        <div x-show="$wire.activeModalTab === 'history'" class="space-y-4">
                            <div class="bg-white p-4 rounded-xl border border-gray-200 space-y-3">
                                <p class="text-[11px] font-black text-gray-900 uppercase tracking-widest border-b pb-1.5 flex items-center justify-between">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="pt-2 border-t border-dashed border-gray-200 mt-2">
                                        <p class="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-1.5 px-1 flex items-center gap-1">
                                        ...
                                        <div class="grid grid-cols-12 gap-1.5 bg-indigo-50/50 p-2 rounded-lg border border-indigo-100/50">
                                            ...
                                            <div class="col-span-2 flex items-end pb-0.5">
                                                <button type="button" wire:click="addManualSurcharge" class="w-full bg-indigo-600 text-white rounded text-[11px] font-black uppercase py-1.5 hover:bg-indigo-700 shadow-sm active:scale-95 transition-all">Lưu</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="space-y-1.5 h-48 overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-gray-200">
                                        ...
                                        <div class="group flex items-center gap-2 bg-white px-2 py-1.5 rounded-lg border border-gray-100 hover:border-indigo-200 shadow-sm transition-all animate-fadeIn">
                                            <div class="flex-1">
                                                <div class="flex justify-between items-center mb-0.5">
                                                    <p class="text-[12px] font-black text-gray-900 uppercase tracking-tight">{{ $log['service_name'] }}</p>
                                                    <p class="text-[12px] font-black text-indigo-600">{{ number_format($log['total_amount'], 0, ',', '.') }}đ</p>
                                                </div>
                                                <div class="flex gap-2 text-[10px] text-gray-900 font-bold items-center">
                                                    ...
                                                    @if(!empty($log['notes']))
                                                        <span class="text-gray-500 italic truncate max-w-[100px] border-l pl-2">"{{ $log['notes'] }}"</span>
                                                    @endif
                                                </div>
                                            </div>
                                    ...
                                    @empty
                                        <p class="py-4 text-center text-gray-500 text-[12px] italic">Chưa có lịch sử</p>
                                    @endforelse
                                </div>
                            </div>
                        </div>
