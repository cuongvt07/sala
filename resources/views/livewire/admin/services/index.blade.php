<div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50/50">
                <tr>
                    ...
                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Đơn giá</th>
                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Đơn vị</th>
    ...
    <x-ui.modal name="showModal" :title="$editingServiceId ? 'Chỉnh sửa Dịch vụ' : 'Thêm Dịch vụ mới'">
        <form wire:submit="save" class="space-y-4">
            <x-ui.input label="Tên dịch vụ" id="name" wire:model="name" :error="$errors->first('name')" required placeholder="VD: Điện, Nước, Wifi..." />
            
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1">
                    <label class="block text-sm font-semibold text-gray-700">Đơn giá (VNĐ)</label>
                    <input 
                        type="text" 
                        wire:model="unit_price" 
                        class="block w-full rounded-lg border-gray-200 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 shadow-sm"
            ...
            <x-ui.input label="Tên đơn vị" id="unit_name" wire:model="unit_name" :error="$errors->first('unit_name')" placeholder="VD: kWh, m3, Tháng, Lần..." />

            <x-ui.input label="Mô tả" id="description" wire:model="description" :error="$errors->first('description')" />
